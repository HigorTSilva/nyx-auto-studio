...
/**
 * Reveal-on-scroll that only resets once the element leaves entirely through
 * the bottom of the viewport (scrolling back up past it). Leaving through the
 * top during normal downward scrolling keeps the content visible as-is, so
 * nothing disappears while reading — the reveal only replays if you scroll
 * back down into the section again.
 */
...
/**
 * Pulls an element a fraction of the distance toward the cursor while hovered,
 * spring-released back to rest on leave. Range is bounded by `strength`, not clamped,
 * so keep it low (0.2–0.4) to stay inside the 10–20px feel the buttons use.
 */
