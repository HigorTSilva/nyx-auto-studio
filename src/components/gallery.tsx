...
function StaticProjectsRow({ onOpen }: { onOpen: (index: number) => void }) {
  return (
    <section className="bg-onyx px-6 pb-16 pt-4 sm:px-10 sm:py-10 lg:px-16">
      <div className="mx-auto flex max-w-6xl gap-4 overflow-x-auto pb-2">
        {PROJECTS.map((project, index) => (
          <button
            ...
            aria-label={`Abrir álbum: ${project.service}`}
            className="relative aspect-3/2 w-80 shrink-0 cursor-pointer overflow-hidden rounded-xl text-left sm:w-64"
          >
            <Image src={project.photos[0].src} alt={project.photos[0].alt} fill className="object-cover" sizes="(min-width: 640px) 256px, 320px" />
...
export function Gallery() {
  ...
  return (
    <>
      <section id="projetos" className="bg-onyx px-6 pb-2 pt-24 sm:px-10 sm:pb-8 lg:px-16">
        <motion.div
          ref={headerRef}
          initial="hidden"
